...
// "0:start:0" means function 0 starts from the very beginning of time 0. "0:end:0" means function 0 ends to the very end of time 0.
    ...
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        for(String log : logs){
            ...
            if(arr[1].equals("start")){
                if(!stack.isEmpty()){
                    res[stack.peek()] += Integer.parseInt(arr[2]) - preStartTime;
                }
                preStartTime = Integer.parseInt(arr[2]);
                stack.push(Integer.parseInt(arr[0])); 
            }
            else{ // end 情况用下一开始时刻
                if(!stack.isEmpty()){
                    res[stack.peek()] += Integer.parseInt(arr[2]) + 1 - preStartTime;
                }
                preStartTime = Integer.parseInt(arr[2]) + 1;
                stack.pop();
            }
        }
        return res;
