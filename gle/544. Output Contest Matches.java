...
//  We'll use parentheses('(', ')') and commas(',') to represent the contest team pairing - parentheses('(' , ')') for pairing and commas(',') for partition. During the pairing process in each round,
...
class Solution {
    public String findContestMatch(int n) {
        ...
        return recursion(arr , arr.length - 1)[0];
    }
    private String[] recursion(String[] arr, int e){
        if(e <= 0){
            return arr;
        }
        int l = 0 , r = e;
...
class Solution {
    public String findContestMatch(int n) {
        ...
        return recursion(list , list.size() - 1).get(0);
    }
    private List<String> recursion(List<String> list , int e){
        if(e <= 0){
            return list;
        }
        int l = 0 , r = e;
