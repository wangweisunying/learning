...
// to the same address without dots in the local name.  For example, "alice.z@leetcode.com" and "alicez@leetcode.com" 
...
public:
    int numUniqueEmails(vector<string>& emails) {
        ...
        for(string e : emails){
            int pivot = e.find_first_of('@');
            string name = e.substr(0 , pivot);
            string domain = e.substr(pivot);
